...
static void parse_output(speed_test_result_t *st, char *buf) {
  double val = 0;
...
static void finish_test(speed_test_result_t *st) {
  ...
  int total = 0;
  for (;;) {
    int n = (int)read(st_pipe_fd, buf + total, sizeof(buf) - 1 - (size_t)total);
    if (n <= 0)
      break;
    total += n;
  }
  ...
  if (st->download_mbps > 0 || st->upload_mbps > 0) {
    ...
    snprintf(st->timestamp, sizeof(st->timestamp), "%02d:%02d", tm->tm_hour,
             tm->tm_min);
    save_latest(st);
    update_history(st);
    append_history(st);
