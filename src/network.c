...
static int find_wifi_iface(char *out, size_t outsz) {
  ...
  while ((ent = readdir(d)) != NULL) {
    ...
    char path[PATH_BUF];
    snprintf(path, sizeof(path), "%s/%s/wireless", PATH_SYS_NET, ent->d_name);
...
static void read_iw_link(network_info_t *ni) {
  char cmd[CMD_BUF];
  snprintf(cmd, sizeof(cmd), CMD_IW_LINK, ni->iface);
...
static void read_ip_addr(network_info_t *ni) {
  char cmd[CMD_BUF];
  snprintf(cmd, sizeof(cmd), CMD_IP_ADDR, ni->iface);
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (!inet)
      continue;
    inet += 5;
    while (*inet && isspace((unsigned char)*inet))
      inet++;
...
void network_read(network_info_t *ni) { memset(ni, 0, sizeof(*ni)); }
